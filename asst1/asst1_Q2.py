...
#up there is the assignment, below is just testing
if __name__ == "__main__":    
    def myfunc (x):
        return x * (x-2) * np.exp(1/2 * x)
    
    def myfuncprimed (x):
        return 1/2 * (x**2 + 2*x-4) * np.exp(1/2*x)
    
    xvals = np.linspace(-3,3,100)
    ndiffvales = ndiff(myfunc, xvals)
    
    plt.plot(xvals, myfunc(xvals), xvals, myfuncprimed(xvals), xvals, ndiffvales)