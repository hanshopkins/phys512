...
def chi2_comp_probability (chisq_1, chisq_2):
    return np.exp(0.5*(chisq_1-chisq_2))
...
if __name__ == "__main__":
    ...
    nsteps = 5000 #chosen arbitrarily
    ...
    for i in range(1,nsteps):
        if ((i/nsteps)%0.1 == 0): #keeping track of progress
            t2 = time.time()
        ...
        if (m_test[3] > 0.01):
            try:
                ...
                if (chisq_test < chisq_chain[i-1]):
                    chain[i] = m_test
                    chisq_chain[i] = chisq_test
                elif (np.random.rand(1) < chi2_comp_probability(chisq_chain[i-1], chisq_test)):
                    chain[i] = m_test
                    chisq_chain[i] = chisq_test
                else:
                    chain[i] = chain[i-1]
    ...
    t2 = time.time()
    print(t2-t1)
    np.savetxt(r"planck_chain.txt",np.append(np.transpose([chisq_chain]), chain, axis = 1))